...
def setText():
    ...
    w.delete(1.0, tk.END)
    w.insert(tk.END, "import botpapy as bp\n")
    ...
    w.insert(tk.END, "img.isolateColorRange(c1, c2)\n")
...
def updateImg(event=None):
    ...
    global filename
    if filename != None:
        res = cv2.inRange(img, np.asarray(c1), np.asarray(c2))
...
def setupTk(master):
    ...
    c1min = tk.Scale(master, from_=0, to=255, length = 150, orient="horizontal", label="minhue", command=updateImg)
    c2min = tk.Scale(master, from_=0, to=255, length = 150, orient="horizontal", label="minsat", command=updateImg)
    c3min = tk.Scale(master, from_=0, to=255, length = 150, orient="horizontal", label="minval", command=updateImg)

    c1max = tk.Scale(master, from_=0, to=180, length = 150, orient="horizontal", label="maxhue", command=updateImg)
    c2max = tk.Scale(master, from_=0, to=255, length = 150, orient="horizontal", label="maxsat", command=updateImg)
    c3max = tk.Scale(master, from_=0, to=255, length = 150, orient="horizontal", label="maxval", command=updateImg)
    c1max.set(180)
    c2max.set(255)
    c3max.set(255)

    w = tk.Text(master, height=6, borderwidth=2)
